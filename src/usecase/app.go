...
func (service *serviceApp) GetAIConfig(_ context.Context) (response domainApp.AIConfigResponse, err error) {
	response.Enabled = config.AIChatbotEnabled
	response.Model = config.AIChatbotModel
	response.SystemPrompt = config.AIChatbotSystemPrompt
	
	// Mask API token for security (only show last 4 characters)
	if config.AIChatbotAPIToken != "" {
		tokenLen := len(config.AIChatbotAPIToken)
		if tokenLen > 4 {
			response.APIToken = "***" + config.AIChatbotAPIToken[tokenLen-4:]
		} else {
			response.APIToken = "***"
		}
	}
...
func (service *serviceApp) UpdateAIConfig(_ context.Context, request domainApp.AIConfigRequest) (response domainApp.AIConfigResponse, err error) {
	if request.Enabled != nil {
		config.AIChatbotEnabled = *request.Enabled
	}
	if request.APIToken != nil && *request.APIToken != "" {
		config.AIChatbotAPIToken = *request.APIToken
	}
	if request.Model != nil && *request.Model != "" {
		config.AIChatbotModel = *request.Model
	}
	if request.SystemPrompt != nil {
		config.AIChatbotSystemPrompt = *request.SystemPrompt
	}
	
	return service.GetAIConfig(context.Background())
}
