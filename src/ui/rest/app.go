...
func InitRestApp(app fiber.Router, service domainApp.IAppUsecase) App {
	...
	app.Get("/app/ai-config", rest.GetAIConfig)
	app.Put("/app/ai-config", rest.UpdateAIConfig)
...
func (handler *App) GetAIConfig(c *fiber.Ctx) error {
	response, err := handler.Service.GetAIConfig(c.UserContext())
	utils.PanicIfNeeded(err)
...
func (handler *App) UpdateAIConfig(c *fiber.Ctx) error {
	var request domainApp.AIConfigRequest
	if err := c.BodyParser(&request); err != nil {
		return fiber.NewError(fiber.StatusBadRequest, "Invalid request body")
	}

	response, err := handler.Service.UpdateAIConfig(c.UserContext(), request)
	utils.PanicIfNeeded(err)
