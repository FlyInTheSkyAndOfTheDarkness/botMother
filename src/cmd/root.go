...
func initEnvConfig() {
	...
	// AI Chatbot settings
	if viper.IsSet("ai_chatbot_enabled") {
		config.AIChatbotEnabled = viper.GetBool("ai_chatbot_enabled")
	}
	if envAIToken := viper.GetString("ai_chatbot_api_token"); envAIToken != "" {
		config.AIChatbotAPIToken = envAIToken
	}
	if envAIModel := viper.GetString("ai_chatbot_model"); envAIModel != "" {
		config.AIChatbotModel = envAIModel
	}
	if envAISystemPrompt := viper.GetString("ai_chatbot_system_prompt"); envAISystemPrompt != "" {
		config.AIChatbotSystemPrompt = envAISystemPrompt
	}
}
...
func initFlags() {
	...
	// AI Chatbot flags
	rootCmd.PersistentFlags().BoolVarP(
		&config.AIChatbotEnabled,
		"ai-chatbot-enabled", "",
		config.AIChatbotEnabled,
		`enable or disable AI chatbot --ai-chatbot-enabled <true/false> | example: --ai-chatbot-enabled=true`,
	)
	rootCmd.PersistentFlags().StringVarP(
		&config.AIChatbotAPIToken,
		"ai-chatbot-api-token", "",
		config.AIChatbotAPIToken,
		`OpenAI API token for AI chatbot --ai-chatbot-api-token <string> | example: --ai-chatbot-api-token="sk-..."`,
	)
	rootCmd.PersistentFlags().StringVarP(
		&config.AIChatbotModel,
		"ai-chatbot-model", "",
		config.AIChatbotModel,
		`OpenAI model for AI chatbot --ai-chatbot-model <string> | example: --ai-chatbot-model="gpt-4o-mini"`,
	)
	rootCmd.PersistentFlags().StringVarP(
		&config.AIChatbotSystemPrompt,
		"ai-chatbot-system-prompt", "",
		config.AIChatbotSystemPrompt,
		`System prompt for AI chatbot --ai-chatbot-system-prompt <string> | example: --ai-chatbot-system-prompt="You are a helpful assistant."`,
	)
}
